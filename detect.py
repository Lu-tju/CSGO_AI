...
def arg_parse():
    ...
    parser.add_argument("--images", dest='images', help="待检测图像目录",
                        default="imgs", type=str)  # images是所有测试图片所在的文件夹
    parser.add_argument("--det", dest='det', help="检测结果保存目录",   # det保存检测结果的目录
...
''' 
args = Namespace(bs=1, cfgfile='cfg/yolov3.cfg', 
                 confidence=0.5,det='det', images='imgs', 
                 nms_thresh=0.4, reso='416', weightsfile='yolov3.weights')
'''
images = args.images    # 目标文件夹=‘imgs’
...
# 加载全部待检测图像
# loaded_ims和[inp_dim for x in range(len(imlist))]是两个列表，loaded_ims是所有图片数组的列表，[inp_dim for x in range(len(imlist))] 遍历imlist长度(即图片的数量)这么多次，每次返回值是图片需resize的输入尺寸inp_dim（如416）

'''
OpenCV 会将图像载入为numpy数组，格式是 [h, w, channel]颜色通道的顺序为 BGR
PyTorch 的图像输入tensor,格式是[batch, channel, h, w]，其通道顺序为 RGB
'''

# 调用了prep_image，调整输入图像大小和格式，归一化，BGR——>RGB
# 输入网络的图像：im_batches: 包含n多个（n幅图）[1, 3, 416, 416]
im_batches = list(map(prep_image, loaded_ims, [inp_dim for x in range(len(imlist))]))  # map函数将对应的元素作为参数传入prep_image函数，最终的所有结果也会组成一个列表(im_batches)，是BxCxHxW（batch*channel*height*width）

# 保存原始图像的信息：im_dim_list = [[602,465]]——>[[602,465,602,465]]
im_dim_list = [(x.shape[1], x.shape[0]) for x in loaded_ims]  # 除了转换后的图像，我们也会创建一个列表im_dim_list用于保存原始图片的维度。一个元素对应一张图片的宽高,opencv读入的图片矩阵对应的是 HxWxC
...
for i, batch in enumerate(im_batches):
    ...
    with torch.no_grad():
        ...
        prediction = model(Variable(batch), CUDA)   # [1,10647,85]
    # NMS:
    ...
    prediction = write_results(prediction, confidence, num_classes, nms_conf=nms_thesh)     # NMS:[3,8]，如果多个图一批，就是[n,8]

    end = time.time()
    # 如果从write_results()返回的一个batch的结果是一个int(0)，表示没有检测到时目标，此时用continue跳过本次循环
    if type(prediction) == int:  # 没有目标
    ...
    prediction[:, 0] += i * batch_size      # 这次预测的图片，对应在所有图片中的序号对应
    ...
    if not write:  # If we have't initialised output
        ...
        output = torch.cat((output, prediction))    # 本批次内所有结果连接
...
# 前面im_dim_list一行的元素为(W,H,W,H)，对应一张图片原始的宽、高，且重复了一次。(W,H,W,H)主要是在后面计算x1,y1,x2,y2各自对应的缩放系数时好对应上。
# 对于此例，狗、自行车、卡车3个目标的话，就重复3次，以对应输出的每个值，进行矩阵相乘，来坐标变换
...
"""
将方框下的的坐标 转换为 原始图片比例下的坐标，分别经过去灰度部分还原和缩放。参见.md  
"""

# scaling_factor = torch.min(416/im_dim_list,1)[0].view(-1,1)#这是源代码，下面是我修改的代码
scaling_factor = torch.min(int(args.reso) / im_dim_list, 1)[0].view(-1, 1)      # min(A,1),1表示按行返回
